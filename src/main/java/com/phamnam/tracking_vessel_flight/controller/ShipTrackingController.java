...
public class ShipTrackingController {
    ...
    @GetMapping
    public ResponseEntity<MyApiResponse<List<ShipTracking>>> getAllShipTrackings() {
    ...
    public ResponseEntity<MyApiResponse<PageResponse<ShipTracking>>> getAllShipTrackingsPaginated(
        ...
        Page<ShipTracking> shipTrackingPage = shipTrackingService.getAllPaginated(pageable);

        return ResponseEntity.ok(MyApiResponse.success(
                PageResponse.fromPage(shipTrackingPage),
                "Ship trackings retrieved successfully"));
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<MyApiResponse<ShipTracking>> getShipTrackingById(@PathVariable Long id) {
    ...
    public ResponseEntity<MyApiResponse<List<ShipTracking>>> getTrackingsByVoyageId(@PathVariable Long voyageId) {
    ...
    @PostMapping
    public ResponseEntity<MyApiResponse<ShipTracking>> createShipTracking(
            @Valid @RequestBody ShipTrackingRequest shipTrackingRequest,
            @RequestParam(required = false) Long userId) {
        ShipTracking savedShipTracking = shipTrackingService.save(shipTrackingRequest, userId);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<MyApiResponse<ShipTracking>> updateShipTracking(
            @PathVariable Long id,
            @Valid @RequestBody ShipTrackingRequest shipTrackingRequest,
            @RequestParam(required = false) Long userId) {
        ShipTracking updatedShipTracking = shipTrackingService.updateShipTracking(id, shipTrackingRequest, userId);
        return ResponseEntity.ok(MyApiResponse.success(updatedShipTracking, "Ship tracking updated successfully"));
    }
    ...
    @PostMapping("/process/{shipId}")
    public ResponseEntity<MyApiResponse<ShipTracking>> processNewTrackingData(
            @PathVariable Long shipId,
            @Valid @RequestBody ShipTrackingRequest shipTrackingRequest,
            @RequestParam(required = false) Long userId) {

        ShipTracking savedShipTracking = shipTrackingService.processNewTrackingData(
                shipId, shipTrackingRequest, userId);

        return new ResponseEntity<>(
                MyApiResponse.success(savedShipTracking, "Ship tracking data processed successfully"),
                HttpStatus.CREATED);
    }
}

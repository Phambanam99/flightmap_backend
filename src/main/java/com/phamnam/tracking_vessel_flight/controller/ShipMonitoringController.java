...
import com.phamnam.tracking_vessel_flight.service.ShipMonitoringService;
...
public class ShipMonitoringController {
    ...
    @GetMapping
    public ResponseEntity<MyApiResponse<List<ShipMonitoring>>> getAllShipMonitoring() {
    ...
    public ResponseEntity<MyApiResponse<PageResponse<ShipMonitoring>>> getAllShipMonitoringPaginated(
        ...
        Page<ShipMonitoring> monitoringPage = shipMonitoringService.getAllPaginated(pageable);

        return ResponseEntity.ok(MyApiResponse.success(
                PageResponse.fromPage(monitoringPage),
                "Ship monitoring entries retrieved successfully"));
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<MyApiResponse<ShipMonitoring>> getShipMonitoringById(@PathVariable Long id) {
    ...
    public ResponseEntity<MyApiResponse<List<ShipMonitoring>>> getShipMonitoringByUserId(@PathVariable Long userId) {
        List<ShipMonitoring> monitoringEntries = shipMonitoringService.getByUserId(userId);
    ...
    public ResponseEntity<MyApiResponse<List<ShipMonitoring>>> getShipMonitoringByShipId(@PathVariable Long shipId) {
        List<ShipMonitoring> monitoringEntries = shipMonitoringService.getByShipId(shipId);
        return ResponseEntity.ok(MyApiResponse.success(monitoringEntries, "Monitoring entries retrieved successfully"));
    }
    ...
    @PostMapping
    public ResponseEntity<MyApiResponse<ShipMonitoring>> createShipMonitoring(
            @Valid @RequestBody ShipMonitoringRequest request) {
        ShipMonitoring savedMonitoring = shipMonitoringService.save(request);
        return new ResponseEntity<>(
                MyApiResponse.success(savedMonitoring, "Ship monitoring entry created successfully"),
                HttpStatus.CREATED);
    }
